...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/user-not-found') {
        setErrors(prev => ({ ...prev, general: translate({ en: 'User not found', pt: 'Usuário não encontrado' }) }));
      } else if (error.code === 'auth/wrong-password') {
        setErrors(prev => ({ ...prev, general: translate({ en: 'Incorrect password', pt: 'Senha incorreta' }) }));
      } else if (error.code === 'auth/invalid-email') {
        setErrors(prev => ({ ...prev, email: translate({ en: 'Invalid email', pt: 'Email inválido' }) }));
      } else {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // O redirecionamento será capturado pelo useEffect getRedirectResult
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md mx-auto space-y-8 relative z-10">
        ...
        {/* Login Form Card - Animated Entrance */}
        <motion.div 
        ...
        >
          <form onSubmit={handleEmailLogin} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className={`w-full px-4 py-3 bg-white/10 border rounded-lg text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-white/20 focus:border-transparent transition-all ${
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center space-x-2 text-white/80 text-sm cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 rounded border-white/20 bg-white/10 text-white focus:ring-2 focus:ring-white/20 focus:ring-offset-0 focus:ring-offset-transparent"
                  disabled={isAuthenticating}
                />
                <span>{translate({ en: 'Remember me', pt: 'Lembre-se' })}</span>
              </label>
            </div>
            ...
            >
              <button
                type="submit"
                disabled={isAuthenticating}
                className="w-full h-12 bg-white text-slate-900 hover:bg-white/90 disabled:bg-white/70 rounded-xl font-medium text-base shadow-lg shadow-white/10 transition-all duration-200 flex items-center justify-center space-x-2"
              >
                {isAuthenticating ? (
                  <>
          ...
          >
            <button
              ...
              disabled={isAuthenticating}
              className="w-full h-12 border border-white/40 text-white hover:bg-white/20 hover:border-white/60 disabled:border-white/20 disabled:text-white/60 rounded-xl font-medium text-base transition-all duration-200 flex items-center justify-center space-x-2 bg-white/5"
            >
              {isAuthenticating ? (
                <>
