...
import { collection, query, onSnapshot, doc, Timestamp, addDoc, serverTimestamp, runTransaction } from "firebase/firestore";
import {
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    unsubscribes.push(onSnapshot(categoriesColRef, (snapshot) => {
        const customCategories = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id })) as DisplayCategory[];
        setAllUserCategories([...defaultCategories, ...customCategories]);
    }, (error) => {
        console.error("Error fetching categories:", error);
        setAllUserCategories(defaultCategories); // Fallback to default
    }));

    // Payment Methods Listener
    const paymentMethodsColRef = collection(db, 'users', userId, 'paymentMethods');
    unsubscribes.push(onSnapshot(paymentMethodsColRef, (snapshot) => {
        const customMethods = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id })) as DisplayPaymentMethod[];
        setAllUserPaymentMethods([...defaultPaymentMethods, ...customMethods]);
    }, (error) => {
